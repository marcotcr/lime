...
from lime.discretize import DecileDiscretizer, EntropyDiscretizer, QuartileDiscretizer
...
class TestLimeTabular(unittest.TestCase):
    ...
    def test_lime_explainer_good_regressor(self):
        ...
        self.assertEqual(
            1, sum([1 if "petal width" in x else 0 for x in keys]), "Petal Width is a major feature"
        )
        self.assertEqual(
            1,
    ...
    def test_lime_explainer_good_regressor_synthetic_data(self):
        X, y = make_classification(
            n_samples=1000, n_features=20, n_informative=2, n_redundant=2, random_state=10
        )
    ...
    def testFeatureNamesAndCategoricalFeats(self):
        ...
        explainer = LimeTabularExplainer(
            training_data=training_data, feature_names=np.array(["one", "two"])
        )
    ...
    def testFeatureValues(self):
        ...
        explainer = LimeTabularExplainer(
            training_data=training_data, categorical_features=[0, 1, 2]
        )
    ...
    def test_lime_explainer_with_data_stats(self):
        ...
        descritizer = QuartileDiscretizer(
            self.train, [], self.feature_names, self.target_names, random_state=20
        )
    ...
    def test_lime_explainer_parallel(self):
        np.random.seed(1)

        rf = RandomForestClassifier(n_estimators=500)
        rf.fit(self.train, self.labels_train)
        n_samples = min(self.test.shape[0], 20)
        ind = np.random.choice(range(self.test.shape[0]), n_samples, replace=False)
        ...
        with mp.Pool(mp.cpu_count() - 1) as p:
            exp_parallel = p.map(explainer_partial, self.test[ind])

        self.assertIsNotNone(exp_parallel)
        self.assertEqual(len(ind), len(exp_parallel))
